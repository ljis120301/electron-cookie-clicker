...
// if the number of cookies clicked is greater than the cost of the upgrade, then purchase the upgrade
  ...
  return (
    <div className="grid grid-rows-[auto_1fr_auto] min-h-screen p-4 md:p-8 bg-amber-50 overflow-x-hidden">
      <h1 className="text-2xl md:text-4xl font-bold text-amber-800 mb-4 md:mb-8 text-center">
        Cookies: {Number.isInteger(cookies) ? cookies : cookies.toFixed(1)}
      </h1>

      <main className="relative flex flex-col lg:flex-row items-center justify-center gap-8 min-h-[60vh]">
        ...
        {/* Upgrades container */}
        <div className="w-full lg:w-80 h-[50vh] lg:h-[70vh] overflow-y-auto bg-amber-50/80 p-0 rounded-lg shadow-lg relative">
          <div className="sticky top-0 z-20 px-4 md:px-6 py-3 bg-gradient-to-b from-amber-50 to-amber-50/95 shadow-md rounded">
      ...
      <footer className="text-amber-600 mt-4 md:mt-8">
        <p className="text-center mb-2 md:mb-4">Click the cookie to earn more cookies!</p>
        
        {/* New purchased upgrades section */}
        <div className="w-full max-w-3xl mx-auto bg-amber-100 rounded-lg p-3 md:p-4">
          <h2 className="text-lg md:text-xl font-bold text-amber-800 mb-2 md:mb-3">Purchased Upgrades</h2>
          <div className="flex flex-wrap gap-2 md:gap-3">
            {Object.entries(upgrades)
              ...
              .map(([key, upgrade]) => (
                <div 
                  key={key}
                  className="flex items-center bg-amber-50 rounded-lg p-2 shadow-sm text-sm md:text-base"
                >
                  <Image
                    src={upgrade.image}
                    alt={upgrade.name}
                  ...
                  <div>
                    ...
                    <p className="text-xs md:text-sm text-amber-700">Owned: {upgrade.count}</p>
                  </div>
                </div>
              ))}
          </div>
